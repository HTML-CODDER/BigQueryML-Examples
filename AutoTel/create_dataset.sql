SELECT
      ...
      ,t.timestamp
FROM (WITH data AS (
                      WITH car_locs AS (SELECT * FROM `autotel_demo.car_locations`),
                           ta_dis AS ( SELECT  *  FROM `autotel_demo.neighborhood_features`  )
...
ORDER BY free_cars DESC) as t JOIN autotel_demo.neighborhood_features as features ON
